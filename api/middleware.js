...
export function isIPWhitelisted(clientIP) {
  ...
  if (!isAllowed) {
    console.log(`[${clientIP}] IP not whitelisted. Normalized: ${normalizedClientIP}, Allowed: ${allowedIPs.join(', ')}`);
  }
...
export function securityMiddleware(handler) {
  return async (request, response) => {
    ...
    if (!isIPWhitelisted(clientIP)) {
      ...
      return response.status(403).json({ error: 'Forbidden: IP address not allowed' });
    }
